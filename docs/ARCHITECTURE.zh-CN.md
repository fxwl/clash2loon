...
所有成功转换的节点都由 `/nodes` 提供，并通过 Loon `[Remote Proxy]` 以 `C2L_Nodes` 链接节点加载。主 `/loon` 配置不再把转换节点写入本地 `[Proxy]`。

策略组采用保守的混合编译：

1. 先基于成功转换的节点和有效代理链解析 Mihomo 动态成员（`include-all` / `include-all-proxies`、`filter`、`exclude-filter`）。
2. 策略组中引用的每个链接节点都由限定来源的精确 `NameRegex` Remote Filter 表示。
3. 顺序安全的连续节点段会合并并分块成 Remote Filter，以减少配置体积。
4. 如果节点段顺序与全局节点顺序不兼容，则改为逐节点精确 Filter，从而保持 YAML 顺序。
5. 精确 `NameRegex` Remote Filter 会限定来源为 `C2L_Nodes`，并自动分块、复用相同节点集合。
6. `compact=off` 强制使用逐节点精确 Filter，但仍然保留 `C2L_Nodes` 链接节点交付。

`/nodes` 现在是 `/loon` 正常运行链路的一部分。刷新 `C2L_Nodes` 即可同步节点增删，上游删除节点后无需继续维护本地 `[Proxy]` 条目。
