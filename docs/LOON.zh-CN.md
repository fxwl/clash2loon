...
主配置会通过 `[Remote Proxy]` 自动引用：

```text
/nodes?token=YOUR_TOKEN
```

因此当你使用 `/loon` 作为完整远程配置时，通常不需要手工再次添加 `/nodes`。

## 为什么节点与主配置分离

大型 Clash 订阅会产生很大的 Loon `[Proxy]` 区段和很长的策略组配置行。

Clash2Loon 会：

1. 在 `/nodes` 发布节点；
2. 创建 Loon `NameRegex` 过滤器；
3. 让 YAML 生成的策略组引用这些过滤器。

这样可以减少重复内容，并提高大型订阅的可管理性。
